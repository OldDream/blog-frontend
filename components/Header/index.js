...
const getIcon = (key) => {
  const hashMap = {
    bug: <BugOutlined className={style['addMargin']} />,
    book: <BookOutlined className={style['addMargin']} />,
    wechat: <WechatOutlined className={style['addMargin']} />,
  };
  return hashMap[key];
};
    ...
    <div className={style.header}>
      <Row type="flex" justify="space-between" align="middle">
        ...
        <Col xs={2} sm={2} md={6} lg={9} xl={8}>
          ...
          >
            <Menu.Item key="0">
              <AppstoreOutlined className={style['addMargin']} />
              全部文章
            </Menu.Item>
            {navArray.map((item) => {
              return (
